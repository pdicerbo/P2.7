...
int main(int argc, char** argv){
    ...
    aux1 = (double*)malloc(fft_h.local_size_grid * sizeof(double));
    aux2 = (double*)malloc(fft_h.local_size_grid * sizeof(double));
	...
	for (i1=0; i1< fft_h.local_size_grid; ++i1)
        ...
        if (istep%30 == 1)
	  {
                ...
                for (i2 = 0; i2 < n2; ++i2)
		  {
                    ...
                    for (i1 = 0; i1 < fft_h.local_n1; ++i1)
		      {
			x1=L1*((double) (fft_h.local_n1_offset + i1) )/n1 - 0.5*L1;
	    ...
	    MPI_Allreduce(MPI_IN_PLACE, &r2mean, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
            ss *= fac;
            r2mean *= fac;
            end = seconds();
	    if(mype == 0)
