    ...
    for( i1 = 0; i1 < fft->local_n1; i1++ ){
      ...
      for( i2 = 0; i2 < n2; i2++ ){
      	for( i3 = 0; i3 < n3; i3++ ){
      	  ...
      	  fft -> fftw_data[i2*n3 + i3] = data_direct[index] + 0.0 * I;
      	}
      }
      
      fftw_execute_dft( fft -> fw_plan_i2D, fft -> fftw_data, fft -> fftw_data);
...
/*     /\*  among i1 dimension *\/ */
    ...
    for( i1 = 0; i1 < fft->local_n1; i1++ ){
      ...
      fftw_execute_dft( fft -> bw_plan_i2D, &(data_rec[index]), fft -> fftw_data);
